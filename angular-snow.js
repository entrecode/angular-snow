...
angular.module('angular-snow', []).directive('ngSnow', function() {
  ...
  /*  function clearCanvas() {
   ...
   renderer.domElement.clearRect(0, 0, windowWidth, windowHeight);
   }
   }*/

  function init(el, flake, flakeCount, mouseControls, gui, scale) {
    if (renderer) {
      renderer.clear();
    }
    ...
    function onMouseWheel(e) {
      ...
      if (e.wheelDelta) {
        cameraZ += e.wheelDelta * 0.02;
      } else if (e.detail) {
        cameraZ += e.detail * 0.2;
      }
    }
    ...
    if (mouseControls) {
      ...
      document.addEventListener('mousemove', function(e) {
        var mouseX  = e.clientX, mouseY = e.clientY, width = windowWidth, halfWidth = width >>
          1, height = windowHeight, halfHeight = height >> 1;
  ...
  return {
    ...
    replace:    true,
    transclude: true,
    template:   '<div class="angular-snow"><ng-transclude></ng-transclude></div>',
